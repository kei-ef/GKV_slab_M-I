...
!-------------------------------------------------------------------------------
  ...
  public    fileio_open_icnt,fileio_close_icnt, &
            ...
            fileio_open_fzv, fileio_close_fzv, & ! K. Fujita added
...
!--------------------------------------
  SUBROUTINE fileio_open_fzv ( path )
!--------------------------------------
    ...
    open( ofzv, file=path//crank//"."//srank//".fzv."//cnew, &
          form="unformatted" )

  END SUBROUTINE fileio_open_fzv

!--------------------------------------
  SUBROUTINE fileio_close_fzv
!--------------------------------------

     close( ofzv )

  END SUBROUTINE fileio_close_fzv
...
!--------------------------------------
  SUBROUTINE fileio_open_dAt ( path )
!--------------------------------------
...
! fujita added
    ...
    open( odAt, file=path//crank//"."//srank//".dAt."//cnew, &
          form="unformatted" )

  END SUBROUTINE fileio_open_dAt

!--------------------------------------
  SUBROUTINE fileio_close_dAt
!--------------------------------------
  ...
  END SUBROUTINE fileio_close_dAt
...
!--------------------------------------
  SUBROUTINE fileio_write_dAt ( dAt, time )
!--------------------------------------

! fujita added

    complex(kind=DP), intent(in), &
         dimension(-nx:nx,0:ny,-nz:nz-1) :: dAt
    real(kind=DP), intent(in) :: time

    !- OUTPUT binary data phi/"*.Al.*"
    if ( ranks == 0 .AND. vel_rank == 0 ) then
       write( unit=odAt  ) time, dAt

       call flush(odAt)
    end if

  END SUBROUTINE fileio_write_dAt
...
!--------------------------------------
    ...
    write( unit=otrn ) time, entrpy, fenegy, menegy,    &
         ...
         pflux_es, pflux_em, eflux_es, eflux_em, dentdt, entflx, exbflx, udotE, npint
