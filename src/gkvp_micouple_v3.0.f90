...
!--------------------------------------
  SUBROUTINE micouple_prep( mxi, myi, omi, kzi, lz )
!--------------------------------------

    complex(kind=DP), intent(out) :: omi, kzi
    real(kind=DP), intent(in) :: lz
...
! Keep electron FLR
      ...
      read(inml,nml=mip)
        ...
        if( nu_H > 0._DP ) then
          dk = ky(1)-ky(0)
          nu_H = nu_H / ( (dk*nx)**2+(dk*global_ny)**2 ) ** hvpwr
        end if
        ...
        write( olog, * ) " # basic nu_H unit            = ", nu_H
        write( olog, * ) " # ( nu_H is adjusted by dt^-1 )"
        write( olog, * ) ""
      ...
      read(inml,nml=ionop)
        ...
        rho2R0 = rho2L * 2._DP * lz / zl

! fujita modified (v3.0)
        if( dref > 0._DP ) then
           dref = rho2R0 ! for parallel nonlinear term
           write( olog, * ) " !! dref is re-set by M-I coupling parameters"
           write( olog, * ) " # dref = rho/R0        = ", dref
        else
           write( olog, * ) " # Parallel nonlinear term is off "
...
!... Find solutions of the dispersion relation
        if ( inum == 1 ) then
          call find_roots( mxi, myi, omi, kzi, lz ) 
        end if
...
! fujita added (v2.0): poisions to write out f(x,y,z,vz,mu) 
      if( np > 0 ) then
...
!      if (trim(z_calc) == "cf4") then 
      else if (trim(z_calc) == "onesided") then
...
!>> hyper viscosity for electrons (fujita added)
      if( ranks==0 .and. nu_H>0._DP ) then
!      if( nu_H>0._DP ) then
        do iv = 1, 2*nv
          do iz = -nz, nz-1
            do my = ist_y, iend_y
              do mx = -nx, nx
                 lf(mx,my,iz,iv) = lf(mx,my,iz,iv) &
                                 - ksq(mx,my,iz) ** hvpwr * ff(mx,my,iz,iv) * ( nu_H / dt ) 
              end do
            end do
          end do
        end do
...
!--------------------------------------
  SUBROUTINE micouple_advnc( hh, ff, phi, Al, istep )
!--------------------------------------
        ...
        do it=1, it_max
          ...
          do my = ist_y, iend_y
            do mx = -nx, nx
              ...
              if ( abs(iphi_ref(mx,my)) > 1.d-12 ) then
                diff = abs( iphi(mx,my) / iphi_ref(mx,my) - 1._DP )
              else
                diff = abs( iphi(mx,my) - iphi_ref(mx,my) )
              end if
              if( diff>diff_max0 ) diff_max0 = diff
...
!                ff(mx,my,iz,iv,im) = ff(mx,my,iz,iv,im)  &
!                                   + ( idns_new(mx,my) - idns_old(mx,my) ) * fmx(iz,iv,im)
...
!--------------------------------------
   SUBROUTINE find_roots( mxin, myin, omg_out, kz_out, lz )
!--------------------------------------
!
!  Note:
!  - Calculation is performed in the MHD unit. 
!  - L, vA, etc are unity in the MHD normalization
!
   complex(kind=DP), intent(out) :: omg_out, kz_out
   real(kind=DP), intent(in) :: lz
   integer, intent(in) :: mxin, myin

   complex(kind=DP), allocatable :: sol(:), kz(:)
   complex(kind=DP) :: dum0
   complex(kind=DP) :: guess(root_max)
   complex(kind=DP) :: sol_tmp, kz_tmp
   real(kind=DP) :: k_unit, kx_pass, ky_pass
   integer :: k
   integer :: cnt0, cnt1, root, sflg
   integer :: ierr

   k_unit   = kymin_g / rho2L ! Normalize k by L, not by rho
   guess(:) = ( 0._DP, 0._DP )

   kx_pass = k_unit * real( mxin, kind=DP)
   ky_pass = k_unit * real( myin, kind=DP )

   allocate(sol(root_max))
   allocate(kz(root_max))

   sol(:) = ( 0._DP, 0._DP ) ! solutions of omega for given kperp
   kz(:)  = ( 0._DP, 0._DP ) ! solutions of kpara for given kperp

   cnt0 = 0
   cnt1 = 0

   call guess_roots( guess, kx_pass, ky_pass, cnt0 ) ! get z_guess
   cnt0 = min( cnt0, root_max ) 

! put guess and find acutual roots
   do root=1, cnt0
     call solv( guess(root), kx_pass, ky_pass, sol_tmp, kz_tmp, sflg )
...
! convert omg[v_A/L] to omg[v_A/l] by multiplying zl=l/L
   sol(:) = sol(:) * zl
   kz(:)  = kz(:) * zl

! select most unstable root
   omg_out = ( 0._DP, 0._DP )
   
   do root=1, cnt1
     if( aimag( sol(root) - omg_out ) > 0._DP ) then
       omg_out = sol(root)
       kz_out  = kz(root)
     end if
   end do

   !converting to values in gkv unit
   omg_out = omg_out * valf / ( 2._DP * lz ) !twopi 
   kz_out  = kz_out / ( 2._DP * lz )  ! twopi
...
!--------------------------------------
   SUBROUTINE guess_roots( guess, kx_pass, ky_pass, cnt0 )
!--------------------------------------

!  Search each vicinity of multiple roots of f=0

    implicit none
    complex(kind=DP), intent(out) :: guess(root_max)
    real(kind=DP),intent(in) :: kx_pass, ky_pass
    integer,intent(out) :: cnt0
    ...
    integer :: i, j
      ...
      do i=1,imax
        ...
        do j=1, 2*imax
          z2 = z2 + dz2
          z  = z1 + ui*z2 
          call fun_iono( z, f, kx_pass, ky_pass, kz )
...
!
!----------------------------------------------
  SUBROUTINE solv( guess, kx, ky, sol_omg, sol_kz, sflg )
!----------------------------------------------
    
    complex(kind=DP), intent(in)  :: guess
    complex(kind=DP), intent(out) :: sol_omg, sol_kz
    real(kind=DP), intent(in)     :: kx, ky
    integer, intent(out) :: sflg

    complex(kind=DP) :: kz
    complex(kind=DP) :: f, df, z, z_new
    real(kind=DP)    :: err, tol
    integer :: i, imax
      ...
      do i=1, imax
        call  fun_iono( z,  f, kx, ky, kz )
        call dfun_iono( z, df, kx, ky )

        z_new = z - f / df
        err   = abs( z_new - z ) / abs( z )
        z     = z_new
  ...
  END SUBROUTINE solv
!----------------------------------------------
!     ionospheric dispersion relation
!  
!----------------------------------------------
  SUBROUTINE fun_iono( z, f, kx, ky, kzout )
    ...
    real(kind=DP)    :: fctg, bs, gam0i, kme
    real(kind=DP)    :: kperp

      kperp =  sqrt( kx**2 + ky**2 ) 
      kme   =  e0 * ( mp * ky - mh * kx ) ! kperp*M*E0
      ...
      f   = ( z + ui * 2._DP * alpha ) * ( 1._DP + zres ) &
            + ui * kperp**2 * dperp - kme 

      kzout = kz
 
    return

  END SUBROUTINE fun_iono

!----------------------------------------------
!     finite diffence function of f
!
!----------------------------------------------
  SUBROUTINE dfun_iono( z, df, kx, ky )

    complex(kind=DP), intent(in)  :: z
    complex(kind=DP), intent(out) :: df
    real(kind=DP), intent(in)     :: kx, ky
    complex(kind=DP) :: ff1, ff2, z1, z2, kz
    real(kind=DP)    :: dx, d
    
      dx = 1.d-8

      d  = dble(z) * dx
      z1 = z + dcmplx( d*0.5_DP, 0._DP ) ! Omega+dOmega/2
      z2 = z - dcmplx( d*0.5_DP, 0._DP ) ! Omega-dOmega/2

      call fun_iono( z1, ff1, kx, ky, kz )
      call fun_iono( z2, ff2, kx, ky, kz )

      df = ( ff1 - ff2 ) / d

    return

  END SUBROUTINE dfun_iono

!----------------------------------------------
  SUBROUTINE solv_kz( z, kx, ky, kzout )
!----------------------------------------------
!
! Provide frequency omega ( named z here )
! and solve magnetospheric dispersion relation
! to obtain consistent kz 
!
    complex(kind=DP), intent(in)  :: z
    complex(kind=DP), intent(out) :: kzout
    real(kind=DP), intent(in) :: kx, ky

    complex(kind=DP) :: f, df, kz, kznew
    real(kind=DP)    :: err, tol
    integer :: i, imax
      ...
      do i=1, imax
        ...
        call  fun_mgnt( z,  f, kx, ky, kz )
        call dfun_mgnt( z, df, kx, ky, kz )

        kznew  = kz - f / df
        err    = abs( kznew - kz ) / abs( kz )
        kz     = kznew
...
!------------------------------
  SUBROUTINE fun_mgnt( z, f, kx, ky, kz )
!------------------------------
    
!   solve magnetospheric disperion relation
!
    use GKV_math, only: math_g0
    ...
    complex(kind=DP) :: zetae, zetai, vti
    complex(kind=DP) :: fncze, fnczi, fctze, fctzi
    real(kind=DP) :: kperp
    real(kind=DP) :: gam0e, gam0i, fctge, fctgi, bs
    integer :: i
      ...
      call math_g0( bs*Anum(0), gam0e )
      fctge = 1._DP - gam0e

      vti   = sqrt( 2._DP * beta ) ! ion thermal velocity relative to vA inc. sqrt{2}
      zetai = z / kz / vti 
      zetae = zetai * sqrt( Anum(0) )
  
      call calc_disp_func( zetae, fncze )
      fctze = 1._DP + zetae * fncze

      call calc_disp_func( zetai, fnczi )
      fctzi = 1._DP + zetai * fnczi

      f = bs * ( fctgi + fctge ) - ( (z/kz)**2 * fctgi - bs ) * ( gam0e * fctze + gam0i * fctzi )

    return

  END SUBROUTINE fun_mgnt
  
!----------------------------------------------
   SUBROUTINE calc_disp_func( zeta, zout )
!----------------------------------------------
!
...
!     
    ...
    complex(kind=DP), intent(in)  :: zeta
    integer :: i, sgm

      zout = ( 0._DP, 0._DP )
     
      if ( abs(zeta) < 1.0d-2 ) then
         
        ! approximated formula for |zeta|<<1
        zout = ui * pi**0.5_DP * exp( -zeta**2 ) &
             - 2.0_DP * zeta * ( 1.0_DP - 1.5_DP * zeta**2 )
      
      else if ( abs(zeta) > 10._DP .and. real(zeta)>0._DP ) then 

        if ( aimag(zeta) > 0._DP ) then
          sgm = 0
        else if ( aimag(zeta) == 0._DP ) then
          sgm = 1
        else if ( aimag(zeta) < 0._DP ) then
          sgm = 2
        end if
        ...
        ! approximated formula for |zeta|>>1
        zout = ui * pi**0.5_DP * dble(sgm) * exp( -zeta**2 ) &
               - 1._DP / zeta * ( 1.0_DP + 0.5_DP/zeta**2 + 3.0_DP/4.0_DP/zeta**4 )
       
      else

       ! eight-pole Pade approximation
       do i = 1, 8
         zout = zout + coefb(i) / ( zeta - coefc(i) )
       end do
 ...
 END SUBROUTINE calc_disp_func
  
!----------------------------------------------
!     finite diffence function of f
!
!----------------------------------------------
   SUBROUTINE dfun_mgnt( z, df, kx, ky, kz )
     ...
     real(kind=DP)    :: dx, d
       ...
       d = dble(kz) * dx
       kz1  = kz + dcmplx( d*0.5_DP, 0._DP ) ! kz+dkz/2
       kz2  = kz - dcmplx( d*0.5_DP, 0._DP ) ! kz-dkz/2

       call fun_mgnt( z, ff1, kx, ky, kz1 )
       call fun_mgnt( z, ff2, kx, ky, kz2 )

       df = ( ff1 - ff2 ) / d

    return

  END SUBROUTINE dfun_mgnt
...
!$OMP shared(wwo,wwe,wr1o,wr1e,wr2o,wr2e) &
      ...
      do ibuff = 0, nbuff-1
        do global_my = 0, global_ny
         ...
         !%%% Backward x-FFT (kx,ky)->(ky,x) %%%
          ...
          call dfftw_execute_dft(plan_x_backward(ithd), w1, w2)
...
!          w3(0:global_ny) = ww(0:global_ny,ix,ibuff)
          ...
          call dfftw_execute_dft_c2r(plan_y_backward(ithd), w3, ftmp)
         !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
