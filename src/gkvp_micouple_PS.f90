...
!$OMP end master
    ...
    allocate(ww1o(-nz:nz-1,0:nm,0:nxw_size,0:ny,0:nprocw-1))
    allocate(ww1e(-nz:nz-1,0:nm,0:nxw_size,0:ny,0:nprocw-1))
    allocate(ww2o(-nz:nz-1,0:nm,0:nxw_size,0:ny,0:nprocw-1))
    allocate(ww2e(-nz:nz-1,0:nm,0:nxw_size,0:ny,0:nprocw-1))    
...
!$OMP barrier
      do iv = 1, 2*nv+4
        if (mod(iv,2) == 1) then ! odd
          if (1+3<=iv .and. iv<=2*nv+3) call fout_transpose_x2y(ww1o,ww2o)        ! 5,7,9,...
          if (1+4<=iv .and. iv<=2*nv+4) call fout_reduction(iv-4,ww2o,fout)      ! 5,7,9,...
          if (1+1<=iv .and. iv<=2*nv+1) call fout_transpose_y2x(wc1e,wc2e)       ! 3,5,7,...
          if (1  <=iv .and. iv<=2*nv  ) call fout_pack_ff_y2x(iv,ff,wc1o)        ! 1,3,5,...
          if (1+2<=iv .and. iv<=2*nv+2) call fout_realspcal_y2x(wc2o,ww1o)        ! 3,5,7,... 
         
        else                     ! even
          if (1+3<=iv .and. iv<=2*nv+3) call fout_transpose_x2y(ww1e,ww2e)        ! 6,8,10,...
          if (1+4<=iv .and. iv<=2*nv+4) call fout_reduction(iv-4,ww2e,fout)      ! 6,8,10,...
          if (1+1<=iv .and. iv<=2*nv+1) call fout_transpose_y2x(wc1o,wc2o)       ! 2,4,6,..
          if (1  <=iv .and. iv<=2*nv  ) call fout_pack_ff_y2x(iv,ff,wc1e)        ! 2,4,6,...
          if (1+2<=iv .and. iv<=2*nv+2) call fout_realspcal_y2x(wc2e,ww1e)        ! 4,6,8,...
        end if 
!$OMP barrier
      end do
!$OMP end parallel
...
!--------------------------------------
  SUBROUTINE fout_pack_ff_y2x( iv, fin, wout ) 
!--------------------------------------
...
!$   ithd = omp_get_thread_num()
      ...
      do my = ist_y, iend_y
       do ix = 0, nx
        do im = 0, nm
          do iz = -nz, nz-1
            w1(iz,im,my,ix) = fin(ix,my,iz,iv,im)
          enddo
        enddo
       enddo
      enddo
      ...
      do my = ist_y, iend_y
       do ix = -nx, -1
        do im = 0, nm
          do iz = -nz, nz-1
            w1(iz,im,my,2*nxw+ix) = fin(ix,my,iz,iv,im)
          enddo
        enddo
       enddo
      enddo
...
!!TBI!! !$OMP end do nowait
  ...
  END SUBROUTINE fout_pack_ff_y2x

!--------------------------------------
  SUBROUTINE fout_transpose_y2x ( wwin, wwout )
!--------------------------------------
...
!$OMP end master
  ...
  END SUBROUTINE fout_transpose_y2x


!--------------------------------------
  SUBROUTINE fout_realspcal_y2x ( win, wout )
!--------------------------------------
    !
!    integer, intent(in) :: iv
    complex(kind=DP), intent(in), &
    ...
    real(kind=DP), dimension(-nz:nz-1,0:nm,ist_xw:iend_xw,0:2*nyw-1) :: wf, wtmp
...
!!TBI!! !$OMP do collapse(2) schedule(dynamic,nchunk_zm)
      ...
      call dfftw_execute_dft_c2r(planr_yb_y2x(ithd), w3, wf)
...
!NEC$ NOINTERCHANGE
            do iz = -nz, nz-1
              wtmp(iz,im,ix,iy) = cef * & ! Normalization for 2D Forward FFT
                                  wf(iz,im,ix,iy)
            end do
          end do
        end do
      ...
      call dfftw_execute_dft_r2c(planr_yf_y2x(ithd), wtmp, w3)
      ...
      do irank = 0, nprocw-1
        ...
        do my = ist_y_g_rank, iend_y_g_rank
          do ix = ist_xw, iend_xw
            do im = 0, nm
              do iz = -nz, nz-1
                wout(iz,im,ix,my-ist_y_g_rank,irank) = w3(iz,im,ix,my)
              end do
            end do
          end do
        end do
...
!$OMP end master
  ...
  END SUBROUTINE fout_realspcal_y2x
  
!--------------------------------------
  SUBROUTINE fout_transpose_x2y ( wwin, wwout )
!--------------------------------------
...
!$OMP end master
  ...
  END SUBROUTINE fout_transpose_x2y
