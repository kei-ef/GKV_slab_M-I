...
! M-I coupling part
  use GKV_micouple, only: micouple_init, micouple_close, micouple_out_cntrl
! M-I coupling part
...
! M-I coupling part
      if ( trim(z_bound) == "mi_couple"  ) then
        call micouple_init( phi, Al )
! debug
        write( olog, * ) "# micouple_init completed"
      end if
! debug
! M-I coupling part
...
! M-I coupling part
      if ( trim(z_bound) == "mi_couple"  ) then
        call micouple_out_cntrl ( time, 0 )
      end if
! M-I coupling part
...
! M-I coupling part
      if ( trim(z_bound) == "mi_couple"  ) then
        call micouple_out_cntrl ( time, 1 )
!! debug
!        if( mod(loop,1000) == 0 ) then
!          if( rankv == 0  .and.  rankm == 0  .and.  ranks == 0 ) then
!!            write( olog, * ) "# vertical profile of phi and Al (0,ny/2) at t = ", time
!!              write( olog, fmt="(1p,5e15.7)" ) &
!!                ( zz(iz), phi(0,ny/2,iz), Al(0,ny/2,iz), iz=-nz, nz-1 )
!            do iz = -nz, nz-1
!              write( olog, fmt="(1p,256e15.7)" ) &
!                zz(iz), ( real(phi(0,my,iz)), my = 1, ny )
!!                zz(iz), ( real(Al(0,my,iz)), my = 1, ny )
!!                zz(iz), ( real(phi(0,my,iz)), imag(Al(0,my,iz)), my = ny/2, ny/2 )
!            end do
!            write( olog, * )
!          end if
!
!!            write( olog, * ) "# v_para profile of ff (0,ny/2) at t = ", time
!!            write( olog, fmt="(1p,7e15.7)" ) &
!!              ( vl(iv), ff(0,ny/2,nz-1,iv,0), ff(0,ny/2,nz  ,iv,0), ff(0,ny/2,nz+1,iv,0), iv=1, 2*nv )
!!            write( olog, * )
!
!!            write( olog, * ) "# v_para profile of ff (0,ny/2) at t = ", time
!!            write( olog, fmt="(1p,7e15.7)" ) &
!!              ( real(2*nz*rankz+iz), ff(0,ny/2,iz,nv/2,0), ff(0,ny/2,iz,nv,0), ff(0,ny/2,iz,3*nv/2,0), iz=-nz-nzb, nz-1+nzb )
!!            write( olog, * )
!
!        end if
!! debug
...
! --- fujita modified to 1000 steps
      if (mod(loop+1000,1000) == 0 ) then 
                                           call clock_sta(16)
...
! M-I coupling part
        if ( trim(z_bound) == "mi_couple"  ) then
          call micouple_out_cntrl ( time, 2 )
        end if
! M-I coupling part
