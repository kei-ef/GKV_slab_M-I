...
!--------------------------------------
  SUBROUTINE trans_sum ( ff, phi, Al, neint, nmint, npint )
!--------------------------------------
!     Check the entropy balance equation
    ...
    real(kind=DP), intent(out), &
      dimension(-nx:nx,0:ny) :: neint, nmint, npint
    ...
    integer  ::  mx, my, iz, iv, im
      ...
      allocate( wpp(-nx:nx,0:ny,-nz:nz-1,1:2*nv,0:nm) )
...
!... fujita added to compute {phi, Al} term (M-I v2.0)
!$OMP parallel
      do im = 0, nm
!$OMP do
        do iv = 1, 2*nv
          do iz = -nz, nz-1
            do my = ist_y, iend_y
              do mx = -nx, nx
                wpp(mx,my,iz,iv,im) = fcs(ranks) / Znum(ranks) * wpp(mx,my,iz,iv,im) &
...
!$OMP parallel do
      do my = ist_y, iend_y
        do mx = -nx, nx
          npint(mx,my) = real( wc2(mx,my), kind=DP )
        end do
      end do
