...
!-------------------------------------------------------------------------------
  ...
  use GKV_intgrl, only: intgrl_thet, intgrl_fsrf, &
                        intgrl_v0_moment, intgrl_v2_moment,intgrl_v0_moment_ms
  ...
  use GKV_micouple, only: micouple_bndry_bound_e, wrt_fxyz
...
! K. Fujita added 
    complex(kind=DP), dimension(:,:,:), allocatable :: dAt
    real(kind=DP), dimension(-nx:nx,0:ny) :: dentdt, entflx, exbflx, udotE, npint
    real(kind=DP) :: dm
    integer :: iz
...
! ... fujita added
        ...
        !fj end 202011
        allocate( dAt(-nx:nx,0:ny,-nz:nz-1) )
        call fld_emfield_hh ( dh, dAt )
        call fileio_write_dAt( dAt, time ) ! fujita added
        deallocate( dAt )
        ...
        call balance ( ff, phi, Al, dh, cf, time,                          &
                       ...
                       dentdt, entflx, exbflx, udotE, npint )
        ...
        call fileio_write_trn( entrpy, fenegy, menegy,     &
             peint, pmint, neint, nmint, dcd, &
             pflux_es, pflux_em, eflux_es, eflux_em,       &
             dentdt, entflx, exbflx, udotE, npint,         & ! fujita added
             time )
        !fj end 202011
...
!     Check the entropy balance equation
    ...
    complex(kind=DP), dimension(-nx:nx,0:ny)            :: wc2, wc4
...
!... fujita added
    real(kind=DP), dimension(-nx:nx,0:ny) :: entflx, dentdt, exbflx, udotE, npint
    real(kind=DP) :: entflx_nz, entflx_zf, dentdt_nz, dentdt_zf
    real(kind=DP) :: exbflx_nz, exbflx_zf, udotE_nz, udotE_zf
    real(kind=DP) :: entflx_wk, dentdt_wk, exbflx_wk, udotE_wk
    real(kind=DP) :: npint_nz, npint_zf, npint_wk
    real(kind=DP) :: srfdz 
!
    complex(kind=DP), dimension(-nx:nx,0:ny,-nz:nz-1,1:2*nv,0:nm) :: vdfdz
    complex(kind=DP), dimension(-nx:nx,0:ny,-nz:nz-1,0:nm) :: dpdz

!...
    integer :: mx, my, iz, iv, im

      srfdz = twopi !pi * cfsrf / real( nz * ( nprocz-1 ) )
      call calc_dfdz( ff, phi, vdfdz, dpdz )
...
! --- \begin{nonlinear_interaction}
      ...
      if( trim(calc_type) == "nonlinear" ) then
        ...
        call trans_sum ( ff, phi, Al, neint, nmint, npint )
...
! fujita added: {phi,psi} term
!$OMP parallel do reduction(+:npint_nz)
        do my = ist1_y, iend_y
          do mx = -nx, nx
            npint_nz = npint_nz + npint(mx,my)
          end do
        end do
        npint_wk = 0._DP
        call MPI_Allreduce( npint_nz, npint_wk, 1, MPI_DOUBLE_PRECISION, &
                              MPI_SUM, fft_comm_world, ierr_mpi )
        npint_nz = 2._DP * npint_wk

        if( rankw == 0 )  then
          my   = 0
!$OMP parallel do reduction(+:npint_zf)
            do mx = 0, nx
              npint_zf = npint_zf + npint(mx,my)
            end do
        endif
        npint_zf = 2._DP * npint_zf
...
! fujita added: dS/dt
! --- \begin{time_derivative_of_entropy_calculation}
        ...
        do iv = 1, 2*nv
          do iz = -nz, nz-1! nz-1
            do my = ist_y, iend_y
              do mx = -nx, nx
                wf(mx,my,iz,iv,im) =                                     &
                        dh(mx,my,iz,iv,im) * conjg( ff(mx,my,iz,iv,im) ) &
                        / fmx(iz,iv,im)
              end do
            end do
          end do
...
!$OMP parallel do
      do my = ist_y, iend_y
        do mx = -nx, nx
          dentdt(mx,my) = fcs(ranks) * tau(ranks) / Znum(ranks) * real( wc2(mx,my), kind=DP ) &
                          + pmint(mx,my)
        end do
      end do

!$OMP parallel do reduction(+:dentdt_nz)
      do my = ist1_y, iend_y
        do mx = -nx, nx
          dentdt_nz = dentdt_nz + dentdt(mx,my)
        end do
      end do
      dentdt_wk  =   0._DP
      call MPI_Allreduce( dentdt_nz, dentdt_wk, 1, MPI_DOUBLE_PRECISION, &
                          MPI_SUM, fft_comm_world, ierr_mpi )
      dentdt_nz = 2._DP * dentdt_wk

      if( rankw == 0 )  then
        my   = 0
          do mx = 0, nx
            dentdt_zf = dentdt_zf + dentdt(mx,my)
          end do
      endif
      dentdt_zf = 2._DP * dentdt_zf

! --- \end{time_derivative_of_entropy_calculation}

! --- \begin{entropy_flux calculation}
        ...
        do iv = 1, 2*nv
          do iz = -nz, nz-1
            do my = ist_y, iend_y
              do mx = -nx, nx
                wf(mx,my,iz,iv,im) =                                     &
                        vdfdz(mx,my,iz,iv,im) * conjg( ff(mx,my,iz,iv,im) ) / fmx(iz,iv,im) 
              end do
            end do
          end do
        end do
...
!$OMP parallel do
      do my = ist_y, iend_y
        do mx = -nx, nx
          entflx(mx,my) = fcs(ranks) * tau(ranks) / Znum(ranks) * real( wc2(mx,my), kind=DP ) 
        end do
      end do

!$OMP parallel do reduction(+:entflx_nz)
      do my = ist1_y, iend_y
        do mx = -nx, nx
          entflx_nz = entflx_nz + entflx(mx,my)
        end do
      end do
      entflx_wk  =   0._DP
      call MPI_Allreduce( entflx_nz, entflx_wk, 1, MPI_DOUBLE_PRECISION, &
                          MPI_SUM, fft_comm_world, ierr_mpi )
      entflx_nz = 2._DP * entflx_wk

      if( rankw == 0 )  then
        my   = 0
          do mx = 0, nx
            entflx_zf = entflx_zf + entflx(mx,my)
          end do
      endif
      entflx_zf = 2._DP * entflx_zf

! --- \end{entropy_flux_calculation}


! --- \begin{Poynting_flux_calculation}
        ...
        do iv = 1, 2*nv
          do iz = -nz, nz-1
            do my = ist_y, iend_y
              do mx = -nx, nx
                wf(mx,my,iz,iv,im) = vl(iv) * ff(mx,my,iz,iv,im) * j0(mx,my,iz,im) &
                                   * conjg( phi(mx,my,iz) ) * sgn(ranks) * fcs(ranks) * sqrt( tau(ranks) / Anum(ranks) )
              end do
            end do
          end do
        end do
...
!$OMP end parallel
     ...
     call intgrl_v0_moment_ms( wf, wc3 )
...
! --- reality condition
      if ( rankw == 0 ) then
        my = 0
          do mx = 0, nx
            wc4(-mx,my) = conjg( wc4(mx,my) )
          end do
      end if
  
      call MPI_Allreduce( wc4, wc2, nxy, MPI_DOUBLE_COMPLEX, &
                          MPI_SUM, zsp_comm_world, ierr_mpi )

!$OMP parallel do
      do my = ist_y, iend_y
        do mx = -nx, nx
          exbflx(mx,my) = real( wc2(mx,my), kind=DP )
        end do
      end do

      exbflx(:,:) = exbflx(:,:) / srfdz ! 2D integral

!$OMP parallel do reduction(+:exbflx_nz)
      do my = ist1_y, iend_y
        do mx = -nx, nx
          exbflx_nz = exbflx_nz + exbflx(mx,my)
        end do
      end do
      exbflx_wk  =   0._DP
      call MPI_Allreduce( exbflx_nz, exbflx_wk, 1, MPI_DOUBLE_PRECISION, &
                          MPI_SUM, fft_comm_world, ierr_mpi )
      exbflx_nz = 2._DP * exbflx_wk

      if( rankw == 0 )  then
        my   = 0
          do mx = 0, nx
            exbflx_zf = exbflx_zf + exbflx(mx,my)
          end do
      endif
      exbflx_zf = 2._DP * exbflx_zf

! --- \begin{ udotE calculation}
        ...
        do iv = 1, 2*nv
          do iz = -nz, nz-1
            do my = ist_y, iend_y
              do mx = -nx, nx
                 wf(mx,my,iz,iv,im) =                                             &
                         vl(iv) * ff(mx,my,iz,iv,im)                            &
                         * sgn(ranks) * Znum(ranks) * conjg( dpdz(mx,my,iz,im) )  &
                         * sqrt( tau(ranks) / Anum(ranks) )
...
!$OMP parallel do
      do my = ist_y, iend_y
        do mx = -nx, nx
          udotE(mx,my) = real( wc2(mx,my), kind=DP ) &
                       - pmint(mx,my) ! add inductive part
        end do
      end do    

!$OMP parallel do reduction(+:udotE_nz)
      do my = ist1_y, iend_y
         do mx = -nx, nx
            udotE_nz = udotE_nz + udotE(mx,my)
         end do
      end do
      udotE_wk  =   0._DP
      call MPI_Allreduce( udotE_nz, udotE_wk, 1, MPI_DOUBLE_PRECISION, &
                            MPI_SUM, fft_comm_world, ierr_mpi )

      udotE_nz = 2._DP * udotE_wk
  
      if( rankw == 0 )  then
         my   = 0
         do mx = 0, nx
            udotE_zf = udotE_zf + udotE(mx,my)
         end do
      endif
      udotE_zf = 2._DP * udotE_zf
      ...
      if ( rank == 0 ) then
        write( unit=obln, fmt="(f15.8, SP, 256ES24.15e3)" ) &
             ...
             dentdt_nz, & ! [22] Time derivative of entropy S_s (ky/=0),
             dentdt_zf, & ! [23] Time derivative of entropy S_s (ky==0),
             entflx_nz, & ! [24] Net in-out entropy flux (ky/=0), ! surface
             entflx_zf, & ! [25] Net in-out entropy flux (ky==0),
             exbflx_nz, & ! [26] Net in-out Poynting flux (ky/=0),
             exbflx_zf, & ! [27] Net in-out Poynting flux (ky==0),
             udotE_nz,  & ! [28] Linear part of Joule dissipation (ky/=0),
             udotE_zf,  & ! [29] Linear part of Joule dissipation (ky==0),
             npint_nz,  & ! [30] Nonlinear part of Joule dissipation (ky/=0),
             npint_zf     ! [31] Nonlinear part of Joule dissipation (ky==0),
...
! --- divergence trap
        if( trim(calc_type) == "nonlinear" ) then
!... modified by Fujita to make it STOPin cases of NaN
          if ( .not. entrpy_nz < 1.d100 ) then
...
!--------------------------------------
  SUBROUTINE calc_dfdz ( ff, phi, vdfdz, dpdz )
!--------------------------------------

   complex(kind=DP), intent(in), &
      dimension(-nx:nx,0:ny,-nz-nzb:nz-1+nzb,1-nvb:2*nv+nvb,0-nvb:nm+nvb) :: ff
   complex(kind=DP), intent(in), dimension(-nx:nx,0:ny,-nz:nz-1) :: phi

   complex(kind=DP), intent(out), &
      dimension(-nx:nx,0:ny,-nz:nz-1,1:2*nv,0:nm) :: vdfdz
   complex(kind=DP), intent(out), dimension(-nx:nx,0:ny,-nz:nz-1,0:nm) :: dpdz

   complex(kind=DP), dimension(-nx:nx,0:ny,-nz-nzb:nz-1+nzb,0:nm) :: psi
   ...
   do iz = -nz, nz-1
     ...
     cefz2(iz)  = 1._DP / ( 60._DP * dpara(iz) ) 
   end do
  
   do im = 0, nm
     do iz=-nz, nz-1
       psi(:,:,iz,im) = phi(:,:,iz) * j0(:,:,iz,im)
     end do
   end do
   ...
   if (trim(z_calc) == "cf4") then
      ...
      do im=0,nm
       ...
       do iv = 1, 2*nv
         do my = ist_y, iend_y
           do mx = -nx, nx
             vdfdz(mx,my,iz,iv,im)                        &
                   = vl(iv) * cefz2(iz) * (              &
                     - 1._DP * ff(mx,my,iz+3,iv,im)      &
                     + 6._DP * ff(mx,my,iz+2,iv,im)      &
                     +18._DP * ff(mx,my,iz+1,iv,im)      &
                     +10._DP * ff(mx,my,iz  ,iv,im)      &
                     -33._DP * ff(mx,my,iz-1,iv,im) )    &
                     * sqrt( tau(ranks) / Anum(ranks) )
           end do
         end do
       end do 
       do my = ist_y, iend_y 
         do mx = -nx, nx
           dpdz(mx,my,iz,im) = cefz2(iz) * (         &
                     - 1._DP * psi(mx,my,iz+3,im)    &
                     + 6._DP * psi(mx,my,iz+2,im)    &
                     +18._DP * psi(mx,my,iz+1,im)    &
                     +10._DP * psi(mx,my,iz  ,im)    &
                     -33._DP * psi(mx,my,iz-1,im) )
         end do
       end do
       ...
       do iv = 1, 2*nv
         do my = ist_y, iend_y
           do mx = -nx, nx
             vdfdz(mx,my,iz,iv,im)                   &
                =  - vl(iv) * cefz(iz) * (          &
                   -  3._DP * ff(mx,my,iz+4,iv,im)     &
                   + 16._DP * ff(mx,my,iz+3,iv,im)     &
                   - 36._DP * ff(mx,my,iz+2,iv,im)     &
                   + 48._DP * ff(mx,my,iz+1,iv,im)     &
                   - 25._DP * ff(mx,my,iz  ,iv,im) )   &
                   * sqrt( tau(ranks) / Anum(ranks) )
           end do
         end do
       end do
       do my = ist_y, iend_y
         do mx = -nx, nx
            dpdz(mx,my,iz,im) = cefz(iz) * (        &
                    -  3._DP * psi(mx,my,iz+4,im)   &
                    + 16._DP * psi(mx,my,iz+3,im)   &
                    - 36._DP * psi(mx,my,iz+2,im)   &
                    + 48._DP * psi(mx,my,iz+1,im)   &
                    - 25._DP * psi(mx,my,iz  ,im) )
         end do
       end do
...
!              dpdz(mx,my,iz,im) = cefz2(iz) * (     &
!                              1._DP * psi(mx,my,iz+3,im)    &
!                           -  9._DP * psi(mx,my,iz+2,im)    &
!                           + 45._DP * psi(mx,my,iz+1,im)    &
!                           - 45._DP * psi(mx,my,iz-1,im)    &
!                           +  9._DP * psi(mx,my,iz-2,im)    &
!                           -  1._DP * psi(mx,my,iz-3,im) )
              dpdz(mx,my,iz,im) = cefz(iz) * (          &
            ...
            do iz = -nz, nz-1
              do my = ist_y, iend_y
                do mx = -nx, nx
                  vdfdz(mx,my,iz,iv,im)                 &
                        = vl(iv) * cefz(iz) * (          &
                         + 2._DP * ff(mx,my,iz+2,iv,im)      &
                         - 4._DP * ff(mx,my,iz+1,iv,im)      &
                         +18._DP * ff(mx,my,iz  ,iv,im)      &
                         -20._DP * ff(mx,my,iz-1,iv,im)      &
                         + 4._DP * ff(mx,my,iz-2,iv,im) )    &
                         * sqrt( tau(ranks) / Anum(ranks) )
                end do
              end do
            end do
...
!--------------------------------------
    ...
    complex(kind=DP), dimension(:,:,:), allocatable :: uj02, dAdt !fujita added
    integer :: mx, my, iz, iv, im
      ...
      allocate(   uj02(-nx:nx,0:ny,-nz:nz-1) )
...
! fujita added to evaluate nonlinear Joule dissipation
!$OMP parallel do collapse(2) private(mx,my,iz,iv,im)
      do im = 0, nm
        do iv = 1, 2*nv
          do iz = -nz, nz-1
            do my = ist_y, iend_y
              do mx = -nx, nx
                wf(mx,my,iz,iv,im) = vl(iv) * ff(mx,my,iz,iv,im) * j0(mx,my,iz,im)**2
              end do
            end do
          end do
        end do
        ...
        do my = ist_y, iend_y
          do mx = -nx, nx
            ...
            qlperp(mx,my,iz) = fcs(ranks) / Znum(ranks) * tau(ranks)  &
              ...
              uj02(mx,my,iz) = fcs(ranks) / Znum(ranks)  &
                             * sqrt( tau(ranks) / Anum(ranks) ) * uj02(mx,my,iz)
          end do
        end do
      end do
