...
! M-I coupling part
  use GKV_micouple, only: micouple_literm_zv, micouple_advnc, micouple_bndry_bound_f, micouple_bndry_bound_e
  use GKV_pnl, only: PNL_term
! M-I coupling part
...
! M-I coupling part
        if ( trim(z_bound) == "mi_couple"  .and.  rankz == 0 ) then
          call micouple_advnc( hh, ff, phi, Al, istep )
        end if
! M-I coupling part
...
!! debug
!        write( olog, * ) "# caldlt_linear completed"
!! debug
...
! fujita added (M-I v3.0)
      ! Parallel nonlinear term
      if( dref > 0._DP ) then
         call PNL_term( ff, psi, chi, dh, cf, ef )
      end if
...
!---$OMP parallel default(none) &
!---$OMP shared(ff,psi,chi,dh,z_bound) &
...
! M-I coupling part
        if ( trim(z_bound) == "mi_couple"  ) then
          call micouple_bndry_bound_f ( ff(:,:,:,:,im) )
        end if
! M-I coupling part
...
! M-I coupling part
        if ( trim(z_bound) == "mi_couple"  ) then
          call micouple_literm_zv ( ff, psi, im, lf )
! M-I coupling part
        else
          call literm_zv_org ( ff, psi, im, lf )
        end if
...
!--------------------------------------
  SUBROUTINE literm_zv_org ( ff, psi, im, lf )
!--------------------------------------
!     (z,v)-derivative of ff
