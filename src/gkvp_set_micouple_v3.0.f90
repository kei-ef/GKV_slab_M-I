...
! for tokamak(eqdsk) equilibrium
  ...
  !fj end 202010
! K. Fujita added 
  use GKV_micouple, only: micouple_prep
...
!      if (trim(z_calc) == "up5") art_diff = 0._DP
      if (trim(z_calc) == "up3" .or. trim(z_calc) == "up5") art_diff = 0._DP
! M-I couple
...
!!      open( odbg, file=trim(f_log)//crank//"."//srank//".dbg."//cnew )
...
! fujita added
      if ( rankw==0 ) then      
         call fileio_open_fzv( trim(f_fxv) )
      end if
!
      ...
      !fj end 202011
      call fileio_open_dAt( trim(f_phi) ) ! fujita added
...
!
     ...
     !fj end 202011
     call fileio_close_dAt ! fujita added
...
!    real(kind=DP) :: lx, ly, lz, kxmin, kymin, dz, mmax, dm, del_c
    real(kind=DP) :: lx, ly, kxmin, kymin, dz, mmax, dm, del_c ! lz is moved to the common variable list
...
! fujita added (M-I v3.0)
                      dref        ! ordering parameter 
...
! fujita added (M-I v3.0)
      ...
      if( trim(equib_type) == "slab") then
        ...
        write( olog, * ) " # the equib_type option is not selectable "
        call flush(olog)
        call MPI_Finalize(ierr_mpi)
        stop
...
! --- set collision frequencies and v-space functions for multi-species GK collision
        ...
        write( olog, * ) 
        write( olog, * ) " # Collision freuency: nu"
        do is1 = 0, ns-1
        do is2 = 0, ns-1
        write( olog, * ) " # a, b, nu_ab = ", is1, is2, ctauiv(is1,is2)
        end do
        end do
        write( olog, * )
        ...
        if ( trim(col_type) == "LB" ) then
          ...
          write( olog, * ) 
          write( olog, * ) " # biased nu* "
          do is1 = 0, ns-1
            do is2 = 0, ns-1
              write( olog, * ) " # a, b, nu*_ab = ", is1, is2, nust(is1,is2)*nu(is1)
            end do
          end do
        end if
        ...
        write( olog, * ) 
        write( olog, * ) " # cfsrf         = ", cfsrf
!        write( olog, * ) " # rootg(1), omg(1) =", rootg(1), omg(1)
!        write( olog, * )
        flush( olog )
...
! K. Fujita added
    complex(kind=DP), allocatable :: fegn(:,:,:,:)
    complex(kind=DP) :: omi, kzi, omkz, chiz, ffct0, ffct1, ffct2
    real(kind=DP) :: ampnl, valf, vts, bb, gg0
    integer :: mxint, myint, myl

    namelist/miint/ampnl, mxint, myint
!
...
! M-I couple
        if( z_bound == "mi_couple" ) then

          read(inml,nml=miint)

          write( olog,* )
          write( olog,* ) " # Initial condition "
          write( olog,* )
          write( olog,* ) " # primary mode:"
          write( olog,* ) " # mx = ", mxint, " my = ", myint
          write( olog,* ) " # relative non-primary mode amplitude = ", ampnl
          write( olog,* )
            
          call micouple_prep( mxint, myint, omi, kzi, lz ) 
          valf = 1._DP / sqrt( beta )

          write( olog, *) " # initial freq omega [vref/Lref] and kpara [1/Lref]"
          write( olog, '(4es16.6)') omi, kzi
          write( olog, *) " # omega / ( kpara * valf )"
          write( olog, '(2es16.6)') omi / kzi / valf
          write( olog, *)

          ! only for the procs that have myint 
          if( (ny+1)*rankw <= myint .and. myint <= (rankw+1)*ny+rankw ) then 
        
            omkz = omi / kzi
            vts  = sqrt( tau( ranks ) / Anum( ranks ) )

            myl = myint - (ny+1)*rankw ! local index
            bb  = ( kx(mxint)**2 + ky(myl)**2 ) * tau(1) * Anum(1) / Znum(1)**2
            call math_g0( bb, gg0 )       
            chiz = bb / ( 1._DP - gg0 ) * valf / omkz  

            ffct0 = sgn(ranks) * fcs(ranks) / tau(ranks) &
                  * valf * chiz * ( 1._DP - omkz / valf / chiz ) * kzi / abs( kzi ) 
              ...
              do im = 0, nm
                do iv = 1, 2*nv
                  do iz = -nz, nz-1
                    ...
                    if( gmy == myint  .and.  mx == mxint ) then             
 

                      !if( dref==0._DP ) then

                      ffct1 = vts * vl(iv) / ( omkz - vts * vl(iv) ) * ffct0 
                      ffct2 = vts * vl(iv) / ( omkz + vts * vl(iv) ) * ffct0 

                      ff(mx,my,iz,iv,im)  = dns1(ranks) * fmx(iz,iv,im)  &
                                          * ( ffct1 * exp(   ui * kzi * ( zz(iz) - lz ) ) &
                                            - ffct2 * exp( - ui * kzi * ( zz(iz) - lz ) ) ) 

                      !else

                      !ff(mx,my,iz,iv,im)   = dns1(ranks) * fmx(iz,iv,im) * ampfc  &
                      !      * ( 1._DP + vl(iv) + zz(iz) )**2                    &
                      !      * ( sin( 0.8_DP*zz(iz) ) + cos( 0.8_DP*zz(iz) ) ) &
                      !      * exp( ui * twopi * rr(mx+nx+1+(2*nx+1)*gmy) )
