...
!-------------------------------------------------------------------------------
  ...
  use RMHDS_pssn, only: ipb_maxvx, ipb_maxvy
! fujita added (M-I v3.0)
  use GKV_pnl, only: pnl_max_eachrank
...
! fujita added
  real(kind=DP), save :: dt_ipb, dt_pnl
...
!--------------------------------------
    ...
    real(kind=DP), save :: tout_dtc
      ...
      if( id == 0 ) then
        ...
        if ( dt < 1.d-10 ) then
          write( olog, * ) "!!! dt is too short !!!"
          call tips_flush
          call MPI_Abort(MPI_COMM_WORLD, ierr_mpi)
        end if

     
! fujita modified to include dt_pnl and dt_ipb (M-I v3.0)
        if ( rankg == 0 ) then
          if( trim(z_bound) == "mi_couple" ) then
             write( unit=odtc, fmt="(f10.5, 1p, 6e15.7)" )  &
               time, dt, dt_limit, dt_nl, dt_pnl, dt_ipb
          else
             write( unit=odtc, fmt="(f10.5, 1p, 5e15.7)" )  &
               time, dt, dt_limit, dt_nl, dt_pnl
          end if
        end if
...
! fujita modified to include dt_pnl and dt_ipb (M-I v3.0)      
          if ( rankg == 0 ) then
            ...
            if( trim(z_bound) == "mi_couple" ) then
              ...
              write( unit=odtc, fmt="(f10.5, 1p, 4e15.7)" )  &
                time, dt, dt_limit, dt_nl, dt_pnl
            end if
...
! fujita added (M-I v3.0)
    real(kind=DP) :: w_pnl_max, w_pnl_max2
    real(kind=DP) :: ipb_max, ipb_max2

      w_nl_max  = max(eps, dx_inv*exb_maxvx_eachrank, dy_inv*exb_maxvy_eachrank)
      w_pnl_max = max(eps, pnl_max_eachrank/dv) 

      call MPI_Allreduce( w_nl_max, w_nl_max2, 1, MPI_DOUBLE_PRECISION, &
                          MPI_MAX, MPI_COMM_WORLD, ierr_mpi )

      call MPI_Allreduce( w_pnl_max, w_pnl_max2, 1, MPI_DOUBLE_PRECISION, &
                          MPI_MAX, MPI_COMM_WORLD, ierr_mpi )

      dt_nl  = courant_num / w_nl_max2
      dt_pnl = courant_num / w_pnl_max2
      dt_limit = min( dt_max, dt_linear, dt_nl, dt_pnl )
!
      if( trim(z_bound) == "mi_couple" ) then

         if( rankz/=0 ) then ! only rankz==0 processes have definite values
           ipb_maxvx = eps
           ipb_maxvy = eps
         end if

         ipb_max = max( dx_inv*ipb_maxvx, dy_inv*ipb_maxvy )
         call MPI_Allreduce( ipb_max, ipb_max2, 1, MPI_DOUBLE_PRECISION, &
                          MPI_MAX, MPI_COMM_WORLD, ierr_mpi )         


         dt_ipb  = courant_num / ipb_max2
         dt_limit = min( dt_limit, dt_ipb ) 
