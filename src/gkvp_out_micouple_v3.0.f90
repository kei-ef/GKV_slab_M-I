...
! K. Fujita added 
    ...
    real(kind=DP), dimension(-nx:nx,0:ny) :: dentdt, entflx, exbflx, udotE, npint, pnint
    real(kind=DP) :: dm
    integer :: iz
...
! fujita added (M-I v2.0) 
        ...
        call balance ( ff, phi, Al, dh, cf, time,                          &
                       ...
                       dentdt, entflx, exbflx, udotE, npint, pnint )
        ...
        call fileio_write_trn( entrpy, fenegy, menegy,     &
             peint, pmint, neint, nmint, dcd, &
             pflux_es, pflux_em, eflux_es, eflux_em,       &
             dentdt, entflx, exbflx, udotE, npint, pnint,  & ! fujita added (M-I v2.0-)
             time )
        !fj end 202011
...
!... fujita added (M-I v2.0)
    real(kind=DP), dimension(-nx:nx,0:ny), intent(out) :: entflx, dentdt, exbflx, udotE, npint
...
!...(M-I v3.0)
    real(kind=DP), dimension(-nx:nx,0:ny), intent(out) :: pnint
    real(kind=DP) :: pnint_nz, pnint_zf, pnint_wk
...
! fujita added (M-I v2.0)
      ...
      npint_zf   = 0._DP
      npint_nz   = 0._DP
! fujita added (M-I v3.0) parallel nonlinear term
      pnint(:,:) = 0._DP
      pnint_zf   = 0._DP
      pnint_nz   = 0._DP
...
! fujita added (M-I v2.0): {phi,psi} term
...
! fujita added (M-I v3.0) parallel nonlinear term
       if( dref > 0._DP ) then
         call pnl_sum( ff, phi, Al, dpdz, dh, pnint )

!$OMP parallel do reduction(+:pnint_nz)
          do my = ist1_y, iend_y
            do mx = -nx, nx
              pnint_nz = pnint_nz + pnint(mx,my)
            end do
          end do
          pnint_wk = 0._DP
          call MPI_Allreduce( pnint_nz, pnint_wk, 1, MPI_DOUBLE_PRECISION, &
                                MPI_SUM, fft_comm_world, ierr_mpi )
          pnint_nz = 2._DP * pnint_wk

          if( rankw == 0 )  then
            my   = 0
!$OMP parallel do reduction(+:pnint_zf)
              do mx = 0, nx
                pnint_zf = pnint_zf + pnint(mx,my)
              end do
          endif
          pnint_zf = 2._DP * pnint_zf
...
! fujita added (M-I v2.0): entropy flux
...
!$OMP parallel do reduction(+:udotE_nz)
      ...
      if ( rank == 0 ) then
        write( unit=obln, fmt="(f15.8, SP, 256ES24.15e3)" ) &
             ...
             pnint_nz,  & ! [32] Parallel nonlinear term (ky/=0),
             pnint_zf
...
! fujiat added (M-I v2.0)
! calculate v*df/fz and dphi/dz
