...
!     Flux-surface average of a real variable wn
    ...
    real(kind=DP), intent(in),  &
      dimension(-nx:nx,0:ny,-nz:iend_z) :: wn
...
!     Calculate the zeroth order velocity moment of wf
    ...
    complex(kind=DP), intent(in),  &
      dimension(-nx:nx,0:ny,-nz:iend_z,1:2*nv,0:nm)   :: wf
    complex(kind=DP), intent(out), &
      dimension(-nx:nx,0:ny,-nz:iend_z)               :: wn
    ...
    integer :: mx, my, iz, iv, im
      ...
      allocate( ww(-nx:nx,0:ny,-nz:iend_z) )

      if( rankm == 0 ) then

!$OMP parallel default(none) &
!$OMP shared(ww,wf,vp,dvp,cef,ist_y,iend_y,iend_z) &
...
!$OMP end parallel
      ...
      call MPI_Allreduce( ww, wn, nxyz1, MPI_DOUBLE_COMPLEX, &
...
!$OMP parallel default(none) &
!$OMP shared(cef,ww,wf,vl,vp,dv,dvp,ist_y,iend_y,iend_z) &
...
!     Calculate the second order velocity moment of wf
    ...
    complex(kind=DP), intent(in),  &
      dimension(-nx:nx,0:ny,-nz:iend_z-1,1:2*nv,0:nm)   :: wf
    complex(kind=DP), intent(out), &
      dimension(-nx:nx,0:ny,-nz:iend_z)               :: wn
